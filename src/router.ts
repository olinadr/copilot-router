...
const CONFIG = {
    port: 3099,
    modelId: "gpt-5-mini", // Internal model passed to Github CLI
...
// ============================================================================
function callCopilot(prompt: string, onData: (chunk: string) => void, onEnd: (err: Error | null, fullText: string) => void) {
...
Bun.serve({
    ...
    async fetch(req) {
        ...
        if (url.pathname === "/v1/models") {
            return Response.json({ object: 'list', data: [{ id: CONFIG.publicRouterModelName, object: 'model', owned_by: 'router-engine' }] }, { headers: corsHeaders });
        }
